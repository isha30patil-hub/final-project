...
export const Route = createFileRoute("/_app/ai-generation/backup")({
...
function AIGenerationPage() {
  ...
  const generate = async () => {
    if (generated.length > 0) {
      toast.warning(
        "Test cases already generated. Approve or reject them here, then go to Execution."
      );
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button
            onClick={generate}
            disabled={loading || !documentId || generated.length > 0}
          >
            {loading ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
          ...
          <Button
            variant="outline"
            onClick={() =>
              toast.warning("Regenerate is disabled to avoid duplicate test cases.")
            }
            disabled={loading}
          >
